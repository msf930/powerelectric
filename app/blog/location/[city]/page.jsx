...
export default async function BlogPage({ searchParams, params }) {
    ...
    const blogPosts = await cachedSanityFetch(
        `blog-posts-location-${city}-${filterSlug}-${page}`,
        BLOG_LIST_POSTS_PAGE_QUERY,
        ...
        <div>
            ...
            <div className={styles.blogPageInner}>
                ...
                <nav className={styles.blogCategoryFilters} aria-label="Filter posts by category">
                    <Link
                        href={blogListHref(blogLocationBase, { page: 1, categorySlug: null })}
                    ...
                    {blogCategories.map((cat) => {
                        ...
                        return (
                            <Link
                                key={cat._id}
                                href={blogListHref(blogLocationBase, { page: 1, categorySlug: slug })}
                ...
                <div className={styles.blogPostsContainer}>
                    {blogPosts.map((post) => (
                        <div className={styles.blogPost} key={post._id}>
                            <Link prefetch={false} href={`/blog/${post.slug.current}${city ? `/${city}` : ""}`}>
                            ...
                            <Link prefetch={false} href={`/blog/${post.slug.current}${city ? `/${city}` : ""}`} className={styles.blogPostLink}>Read More <FaArrowRight /></Link>
                        </div>
                    ))}
                </div>
                {total > 0 && (
                    <nav className={styles.blogPagination} aria-label="Blog pagination">
                        {page > 1 ? (
                            <div className={styles.blogPaginationLinkContainer}>
                                <Link
                                    href={blogListHref(blogLocationBase, {
                                        page: page - 1,
                                        categorySlug,
                                    })}
