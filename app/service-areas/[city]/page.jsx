...
export default async function ServiceAreasPage({ params }) {
    ...
    return (
        <div className={styles.serviceAreasPage}>
            ...
            <div className={styles.serviceAreasPageButtonContainer}>
                <p className={styles.serviceAreasPageButtonContainerTitle}> Currently Viewing Service Area for {city.replace(/-/g, " ")}</p>
                <Link href={`/service-areas`} className={styles.serviceAreasPageButton}>Clear Service Area</Link>
                </div>
            <div className={styles.serviceAreasPageContent}>
                {serviceAreas.map((serviceArea, index) => (
                    <div key={serviceArea._id + index} className={styles.serviceArea}>
